...
export async function DELETE(request: Request, { params }: CourseRouteProps) {
  const { courseId } = await params;
  const supabase = await createClient();
  if (!supabase) {
  ...
  if ((learnerCount ?? 0) > 0 && !body.acknowledgeImpact) {
    return NextResponse.json(
      { error: "This course has active learners.", learnerCount, requiresConfirmation: true },
      { status: 409 },
    );
  }
  ...
  const { error } = await supabase
    ...
    .eq("trader_id", membership.trader_id);
  if (error) {
    return NextResponse.json({ error: "The course could not be deleted." }, { status: 400 });
  }
