...
interface XmVerifyFormProps {
  compact?: boolean
}

export default function XmVerifyForm({ compact = false }: XmVerifyFormProps) {
  ...
  const onSubmit = async (data: VerifyForm) => {
    try {
      ...
      if (result?.type === 'PENDING_SIGNUP') {
        setPendingSignup({
          affiliateCode:
            typeof result.affiliateCode === 'string' ? result.affiliateCode : undefined,
          affiliateLink:
            typeof result.affiliateLink === 'string' ? result.affiliateLink : null,
        })
        toast({
          title: 'Details saved!',
      ...
      >
        <div className="flex items-start gap-3 rounded-lg border border-primary/20 bg-primary/5 p-4">
          ...
          <div className="space-y-2 text-xs leading-relaxed text-muted-foreground">
            <p>
              Your XM Account ID is the number from your XM trading account. We use it to confirm
        ...
        <div className="space-y-2">
          <Label htmlFor="xmAccountId" className="text-xs uppercase tracking-wider text-foreground">
            XM Account ID
          </Label>
          <Input
            id="xmAccountId"
        ...
        >
          {isSubmitting ? 'Verifying…' : 'Verify my XM account'}
        </button>
        ...
        {pendingSignup && (
          ...
          >
            <p className="text-sm text-muted-foreground">
              Your details are saved. Open a new XM account using our referral code:
            </p>
