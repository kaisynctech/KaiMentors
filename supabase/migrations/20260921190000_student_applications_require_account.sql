-- Verification academies only list students who already submitted an XM /
-- broker account ID. New joins require that ID, so mentors never review empty
-- "No account number" rows.
...
create function public.get_student_applications_page(
  ...
  target_require_account boolean default false
)
returns table (
  application_id uuid,
  ...
  broker_account_identifier text,
  total_count bigint
)
language sql
stable
...
as $$
  ...
  where application.trader_id = target_trader_id
    ...
    and (
      not target_require_account
      or length(trim(coalesce(application.trading_account_number, ''))) > 0
      or length(trim(coalesce(application.broker_account_identifier, ''))) > 0
    )
    and (
