...
export async function generateMetadata({ params }: PortalPageProps): Promise<Metadata> {
  ...
  if (customSite) return { title: customSite.title, description: customSite.description ?? customSite.portal.hero_subtitle };
  const website = await loadWebsiteBySlug(slug);
  if (website) {
    const home = website.pages.find((page) => page.is_home);
    return { title: home?.seo_title ?? website.portal.portal_name, description: home?.seo_description ?? website.portal.hero_subtitle };
  }
  const entry = await loadAcademyEntryBySlug(slug);
  return { title: entry?.portal.portal_name ?? "Academy website", description: entry?.portal.academy_description ?? undefined };
}
